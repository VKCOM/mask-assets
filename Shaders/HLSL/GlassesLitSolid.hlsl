    ...
    out float4 oColor : OUTCOLOR0)
{
    ...
    // Get fog factor
        ...
        finalColor = diffColor.rgb * Sample2D(NormalMap, iScreenPos.xy).rgb;

        #ifdef SPECULAR
            float spec = GetSpecular(normal, cCameraPosPS - iWorldPos.xyz, lightDir, cMatSpecColor.a);
            finalColor += diff * lightColor * spec * specColor * cLightColor.a;
        #endif

        #ifdef ENVCUBEMAP
            float4 envMapColor = SampleCube(EnvCubeMap, reflect(iReflectionVec, normal));
            finalColor += cMatEnvMapColor * envMapColor.rgb * envMapColor.a * dot(diffColor.rgb, float3(0.3, 0.59, 0.11));
        #endif
