...
*/
Model @CreateModelPlane(uint hSegment, uint vSegment)
{
    ...
    vb.SetSize((vSegment + 1) * (hSegment + 1), elements);

    uint hVertexNumber = hSegment + 1;

    VectorBuffer vertexBuffer;
    for (uint y = 0; y <= vSegment; y++)
    {
        for (uint x = 0; x < hVertexNumber; x++)
        {
            Vector3 v = Vector3(2.0 * float(x) / hSegment - 1.0, 2.0 * (float(y) / vSegment) - 1.0, 0);
            // Vertex
            vertexBuffer.WriteVector3(v);
            // Texture Coords
            Vector2 tc = Vector2(float(x) / hSegment, 1.0 - float(y) / vSegment);
    ...
    ib.shadowed = true;
    uint indexNumbers = hSegment * vSegment * 6;
    ib.SetSize(indexNumbers, false);

    VectorBuffer indexBuffer;
    for (uint y = 0; y < vSegment; y++)
    {
        for (uint x = 0; x < hSegment; x++)
        {
            //            Print(y * hVertexNumber + x + 1);
            indexBuffer.WriteUShort(y * hVertexNumber + x + 1);
            //            Print(y * hVertexNumber + x);
            indexBuffer.WriteUShort(y * hVertexNumber + x);
            //            Print((y + 1) * hVertexNumber + x);
            indexBuffer.WriteUShort((y + 1) * hVertexNumber + x);

            //            Print(y * hVertexNumber + x + 1);
            indexBuffer.WriteUShort(y * hVertexNumber + x + 1);
            //            Print((y + 1) * hVertexNumber + x);
            indexBuffer.WriteUShort((y + 1) * hVertexNumber + x);
            //            Print((y + 1) * hVertexNumber + x + 1);
            indexBuffer.WriteUShort((y + 1) * hVertexNumber + x + 1);
        }
    }
...
class liquifiedwarp : BaseEffectImpl
{
    ...
    */
    ...
    bool Init(const JSONValue &effect_desc, BaseEffect @parent) override
    {
        ...
        // TODO: need to adapt with aspect ratio
    ...
    bool loadSetup(const JSONValue &jsonSettigns)
    {
        ...
        if (jsonPoints.isArray)
        {
            ...
            for (int i = 0; i < Min(jsonPoints.size, MAXPOINTS); i++)
            {
                ...
                point.type = jsonPoint.Get("type").GetString() == "shift"
                    ? LPT_SHIFT
                    : jsonPoint.Get("type").GetString() == "zoom"
                        ? LPT_ZOOM
                        : LPT_UNKNOWN;
    ...
    void HandleUpdate_v0(StringHash eventType, VariantMap &eventData)
    {
        ...
        for (uint i = 0; i < points.length; i++)
        {
            ...
            if (detectData["PoiMap"].GetVariantMap().Contains(point.anchor))
            {
                ...
                centerBuffer.WriteVector4(Vector4(
                    centerInScreen.x,
                    centerInScreen.y,
                    directionInScreen.x,
                    directionInScreen.y
                ));
                radiusAndTypeAndDebug.WriteVector4(Vector4(
                    point.radius.x * scaleFactor,
                    point.radius.y * scaleFactor,
                    (faceEnabled ? float(point.type) : 0.0),
                    float(point.debug ? 1.0 : 0.0)
                ));
                scaleAngleDirection.WriteVector4(Vector4(
    ...
    void HandleUpdate_v1(StringHash eventType, VariantMap &eventData)
    {
        ...
        // uniform float4 cCenter[MAXPOINTS];
        // uniform float4 cRadiusAndType[MAXPOINTS];  // We use only x and y, z for type
        // uniform float4 cScaleAngleDirection[MAXPOINTS];  // It is: Scale, Angle, UMin, UMax
        VectorBuffer centerBuffer;
        VectorBuffer radiusAndTypeAndDebug;
        VectorBuffer scaleAngleDirection;
